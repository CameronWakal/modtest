...
export default class ModuleClockModel extends Module {
  @service midi;
  @service scheduler;
  ...
  sourceMenuValues = sourceMenuValues;

  @belongsTo('port-value-in', { async: false, inverse: null }) tempoInPort;
  @belongsTo('port-value-in', { async: false, inverse: null }) resInPort;
  @belongsTo('port-event-out', { async: false, inverse: null }) resetOutPort;
  @belongsTo('port-event-out', { async: false, inverse: null }) trigOutPort;
  @attr('string', { defaultValue: 'Internal' }) source;
  ...
  sendEvent(event) {
    if (this.trigOutPort?.isConnected) {
      ...
      this.tickDuration = event.duration;
